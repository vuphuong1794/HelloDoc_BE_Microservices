...
export class PostService {
  ...
  async getAll(
    ...
    try {
        ...
        posts.map(async (post) => {
          try {
            const owner = await firstValueFrom(
              this.usersClient
                .send('user.getuserbyid', post.user.toString())
                .pipe(timeout(3000)),
            );
      ...
      const ownerMap = new Map(
        owners.filter((o) => o !== null).map((o) => [o.postUserId, o.data]),
      );

      // gán lại dữ liệu user
      const postsWithOwners = posts.map((post) => {
        const userInfo = ownerMap.get(post.user.toString()) || null;
        const postWithMedia = this.mediaUrlHelper.constructObjectUrls(post, [
          'media',
        ]);
        return {
          ...postWithMedia,
  ...
  async getAllWithFilter(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Lỗi khi lấy danh sách bài viết với bộ lọc',
      );
    }
  }
    ...
    try {
      ...
      if (!post) {
        throw new NotFoundException(
          `Không tìm thấy bài viết với id ${objectId}`,
        );
      }
      ...
      try {
        ...
        const postWithOwner = {
          ...this.mediaUrlHelper.constructObjectUrls(post.toObject(), [
            'media',
          ]),
  ...
  private async findOwnerById(
    ownerId: string,
  ): Promise<{ model: 'User' | 'Doctor'; data: any }> {
    try {
      let user = await lastValueFrom(
        this.usersClient.send('user.getuserbyid', ownerId).pipe(timeout(3000)),
      );
      //console.log("user nhan duov la " + user);
      if (!user) {
        user = await lastValueFrom(
          this.doctorClient
            .send('doctor.get-by-id', ownerId)
            .pipe(timeout(3000)),
        );
        if (!user) {
      ...
      throw new NotFoundException(
        `Không tìm thấy người dùng với id ${ownerId}`,
      );
    } catch (error) {
  ...
  async getByUserId(
    ...
    try {
      ...
      const postsWithUserInfo = await Promise.all(
        posts.map(async (post) => {
          try {
            ...
            console.error(
              `Error fetching user info for post ${post._id}:`,
              error,
            );
            return {
              ...post.toObject(),
      ...
      throw new InternalServerErrorException(
        'Lỗi khi lấy bài viết của người dùng',
      );
    }
  }
  ...
  async update(id: string, updatePostDto: UpdatePostDto) {
    ...
    try {
      this.logger.log(
        `Updating post ${id} with data:`,
        JSON.stringify(updatePostDto, null, 2),
      );
      ...
      this.logger.log(
        ...
        JSON.stringify((updatedPost as any).toObject(), null, 2),
      );
  ...
  async searchPosts(query: string) {
    ...
    const queryVector = await firstValueFrom(
      this.embeddingClient.send('embedding.generate', query),
    );
  ...
  async generateAndStoreEmbedding(
    ...
    try {
      ...
      if (!textForEmbedding) {
        this.logger.warn(
          `⚠️ No content or keywords to generate embedding for post ${postId}.`,
        );
        return;
      }
      ...
      const hasMongoEmbedding =
        mongoPost.embedding &&
        Array.isArray(mongoPost.embedding) &&
        mongoPost.embedding.length > 0;
      ...
      if (hasMongoEmbedding && !hasQdrantVector) {
        this.logger.warn(
          `⚠️ Qdrant missing vector for post ${postId}. Restoring...`,
        );
      ...
      if (!hasMongoEmbedding && hasQdrantVector) {
        this.logger.warn(
          `⚠️ MongoDB missing embedding for post ${postId}. Restoring from Qdrant...`,
        );
        ...
        this.logger.log(
          `🔄 Restored MongoDB embedding from Qdrant for post ${postId}.`,
        );
        return;
      }
  ...
  async findSimilarPosts(
    ...
    try {
      ...
      if (!qdrantRecord?.result?.vector) {
        this.logger.log(
          `Vector not found in Qdrant for post ${id}. Updating...`,
        );
        await this.updateEmbeddingByPostId(id);
      }
      ...
      this.logger.debug(
        `Qdrant returned ${similarResults.length} similar posts`,
      );

      // 4. Lấy postIds hợp lệ
      const postIds = similarResults.map((r) => r.postId).filter(Boolean);
      ...
      this.logger.log(
        `Loaded ${result.length}/${postIds.length} posts from MongoDB`,
      );

      // 6. Map posts để tra cứu nhanh
      const postMap = new Map(result.map((p) => [p._id.toString(), p]));
      ...
      const finalResult = similarResults
        .map((item) => {
          ...
          return {
            post: this.mediaUrlHelper.constructObjectUrls(post, ['media']),
      ...
      this.logger.error(
        `Error finding similar posts: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async updateEmbeddingByPostId(postId: string): Promise<void> {
    ...
    const post = await this.postModel
      .findById(postId)
      .select('embedding keywords content');
    if (!post) {
  ...
  async updateEmbeddingAsync(): Promise<void> {
    ...
    await this.postModel.updateMany({}, { $set: { embedding: [] } });
    ...
    for (const post of posts) {
      ...
      // Xóa embedding cũ
      await this.postModel.updateOne({ _id: id }, { $set: { embedding: [] } });
